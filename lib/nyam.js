...
exports.post_update = function(opts, callback){
  var message = opts._.slice(1).join(' ')
  
  message = message || fs.readFileSync('/dev/stdin').toString()
  
  var data = {body: message}
  
  oa.post(config.base_url+'/api/v1/messages.json?access_token=' + config.oauth_access_token
    ...
    oa.get(config.base_url+'/api/v1/messages/algo.json?newer_than='+ (config.newer_than ||0) + "&threaded=extended",
        ...
        function(err,json) {
            ...
            for (var i in references) {
                if(references[i].type=='user' && references[i].id) {
                    user_information[references[i].id] = references[i];
                }
                
                if (references[i].type=='group' && references[i].id){
                  groups[references[i].id] = references[i]
                }
